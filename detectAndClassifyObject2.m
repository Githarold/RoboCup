function detectAndClassifyObject2(coordinator)
...
%         imshow(rgbImg);
        ...
        % 객체를 인식하고 Label을 시각화
        [bboxes, scores, labels] = detect(coordinator.DetectorModel2, rgbImg, 'Threshold', 0.7);      % Yolo v4
...
%             imshow(labeledImg);
            ...
            for i=1:coordinator.NumObjs
                ...
                if coordinator.detect_trig==1
                    if bbox_offset>bboxes(i,1) || width-bbox_offset<bboxes(i,1)+bboxes(i,3)
                        objs{i} = [];
                        continue
                      
                    elseif bbox_offset>bboxes(i,2) || height-bbox_offset<bboxes(i,2)+bboxes(i,4)
                        objs{i} = [];
                        continue
                    end
                end                
            ...
            coordinator.Objs = coordinator.Objs(~cellfun('isempty',coordinator.Objs)); %빈 cell 제거 seok
